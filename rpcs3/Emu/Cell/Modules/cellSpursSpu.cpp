...
bool spursKernel1SelectWorkload(SPUThread& spu)
{
	...
	//vm::reservation_op(vm::cast(ctxt->spurs.addr(), HERE), 128, [&]()
	{
		...
		std::memcpy((void*)(spu.offset + 0x100), spurs, 128);
	}//);
...
void spursKernelDispatchWorkload(SPUThread& spu, u64 widAndPollStatus)
{
	...
	std::memcpy((void*)(spu.offset + 0x3FFE0), wklInfoOffset, 0x20);

	// Load the workload to LS
	auto wklInfo = spu._ptr<CellSpurs::WorkloadInfo>(0x3FFE0);
	if (ctxt->wklCurrentAddr != wklInfo->addr)
	{
		...
		case SPURS_IMG_ADDR_TASKSET_PM:
			...
			break;
		default:
			std::memcpy((void*)(spu.offset + 0xA00), wklInfo->addr.get_ptr(), wklInfo->size);
			break;
		}
...
{
	auto ctxt = spu._ptr<SpursKernelContext>(spu.gpr[3]._u32[3]);
...
void spursSysServiceIdleHandler(SPUThread& spu, SpursKernelContext* ctxt)
{
	...
	while (true)
	{
		//vm::reservation_acquire((void*)(spu.offset + 0x100), vm::cast(ctxt->spurs.addr(), HERE), 128);
		auto spurs = spu._ptr<CellSpurs>(0x100);
		...
		//if (vm::reservation_update(vm::cast(ctxt->spurs.addr(), HERE), (void*)(spu.offset + 0x100), 128) && (shouldExit || foundReadyWorkload))
		{
			break;
		}
	}
...
void spursSysServiceMain(SPUThread& spu, u32 pollStatus)
{
	...
	if (ctxt->sysSrvInitialised == 0)
	{
		...
		//vm::reservation_op(ctxt->spurs.ptr(&CellSpurs::wklState1).addr(), 128, [&]()
		{
			...
			std::memcpy((void*)(spu.offset + 0x2D80), spurs->wklState1, 128);
		}//);
...
{
	auto spurs = spu._ptr<CellSpurs>(0x100);
	std::memcpy((void*)(spu.offset + 0x30000), ctxt->spurs->wklInfo1, 0x200);
	if (spurs->flags1 & SF1_32_WORKLOADS)
	{
		std::memcpy((void*)(spu.offset + 0x30200), ctxt->spurs->wklInfo2, 0x200);
	}
	...
	for (u32 i = 0; i < CELL_SPURS_MAX_WORKLOAD; i++)
	{
		auto wklInfo1 = spu._ptr<CellSpurs::WorkloadInfo>(0x30000);
		...
		if (spurs->flags1 & SF1_32_WORKLOADS)
		{
			auto wklInfo2 = spu._ptr<CellSpurs::WorkloadInfo>(0x30200);
...
void spursSysServiceTraceUpdate(SPUThread& spu, SpursKernelContext* ctxt, u32 arg2, u32 arg3, u32 forceNotify)
{
	...
	{
		//vm::reservation_acquire((void*)(spu.offset + 0x80), ctxt->spurs.ptr(&CellSpurs::traceBuffer).addr(), 128);
		...
		{
			std::memcpy((void*)(spu.offset + 0x2C00), vm::base(spurs->traceBuffer.addr() & -0x4), 0x80);
			auto traceBuffer = spu._ptr<CellSpursTraceInfo>(0x2C00);
...
s32 spursTasksetProcessRequest(SPUThread& spu, s32 request, u32* taskId, u32* isWaiting)
{
	...
	//vm::reservation_op(vm::cast(ctxt->taskset.addr(), HERE), 128, [&]()
	{
		...
		std::memcpy((void*)(spu.offset + 0x2700), taskset, 128);
	}//);
...
void spursTasksetOnTaskExit(SPUThread& spu, u64 addr, u32 taskId, s32 exitCode, u64 args)
{
	...
	std::memcpy((void*)(spu.offset + 0x10000), vm::base(addr & -0x80), (addr & 0x7F) << 11);
...
s32 spursTasketSaveTaskContext(SPUThread& spu)
{
	...
	std::memcpy(vm::base(contextSaveStorage), (void*)(spu.offset + 0x2C80), 0x380);
	...
	for (auto i = 6; i < 128; i++)
	{
		if (ls_pattern._bit[i])
		{
			...
			std::memcpy(vm::base(contextSaveStorage + 0x400 + ((i - 6) << 11)), (void*)(spu.offset + CELL_SPURS_TASK_TOP + ((i - 6) << 11)), 0x800);
		}
	}
...
void spursTasksetDispatch(SPUThread& spu)
{
	...
	std::memcpy((void*)(spu.offset + 0x2780), &ctxt->taskset->task_info[taskId], sizeof(CellSpursTaskset::TaskInfo));
	auto taskInfo = spu._ptr<CellSpursTaskset::TaskInfo>(0x2780);
	...
	if (isWaiting == 0)
	{
		...
		std::memset((void*)(spu.offset + CELL_SPURS_TASK_TOP), 0, CELL_SPURS_TASK_BOTTOM - CELL_SPURS_TASK_TOP);
		...
		if ((elfAddr & 5) == 1)
		{
			std::memcpy((void*)(spu.offset + 0x2FC0), &((CellSpursTaskset2*)(ctxt->taskset.get_ptr()))->task_exit_code[taskId], 0x10);
		}
	...
	else
	{
		...
		const u32 contextSaveStorage = vm::cast(taskInfo->context_save_storage_and_alloc_ls_blocks & -0x80, HERE);
		std::memcpy((void*)(spu.offset + 0x2C80), vm::base(contextSaveStorage), 0x380);
		for (auto i = 6; i < 128; i++)
		{
			if (ls_pattern._bit[i])
			{
				// TODO: Combine DMA requests for consecutive blocks into a single request
				std::memcpy((void*)(spu.offset + CELL_SPURS_TASK_TOP + ((i - 6) << 11)), vm::base(contextSaveStorage + 0x400 + ((i - 6) << 11)), 0x800);
			}
		}
...
s32 spursTasksetProcessSyscall(SPUThread& spu, u32 syscallNum, u32 args)
{
	...
	if (incident)
	{
		...
		std::memset((void*)(spu.offset + ctxt->guidAddr), 0, 0x10);
